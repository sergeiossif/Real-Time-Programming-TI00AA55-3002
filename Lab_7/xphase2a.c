...
int main(void)
{
	...
	*/
	...
	while (i < 5) {
		...
		i++;
		if (pid == 0) {
			printf("Child created, running exec, i is %i \n", i);
			
			int error = execl("exphase1b.bin", "exphase1b.bin", buffer1, buffer2, buffer3,  (char*) 0);
			perror("exec fail \n");
